      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-20">
          <FadeIn x={-20} y={0} duration={0.9} className="grid gap-3 sm:grid-cols-[82px_1fr]">
            <div className="order-2 flex gap-2 overflow-x-auto sm:order-1 sm:flex-col">{product.images.map((image, index) => <button key={image} type="button" onClick={() => setActiveImage(index)} className={`h-20 w-16 shrink-0 overflow-hidden border-2 transition-colors sm:h-24 sm:w-[82px] ${activeImage === index ? 'border-ink' : 'border-transparent opacity-55 hover:opacity-100'}`}><img src={image} alt={`${product.name} صورة ${index + 1}`} className="h-full w-full object-cover grayscale" /></button>)}</div>
            <div className="product-detail-image group relative order-1 aspect-[0.86] overflow-hidden bg-fog sm:order-2"><img src={product.images[activeImage]} alt={product.name} className="h-full w-full object-cover grayscale-[0.5] transition duration-700 group-hover:scale-[1.02]" /><div className="absolute inset-x-4 bottom-4 flex justify-between sm:inset-x-6 sm:bottom-6"><span className="bg-paper/85 px-3 py-2 text-xs font-bold">{String(activeImage + 1).padStart(2, '0')} / {String(product.images.length).padStart(2, '0')}</span><div className="flex gap-2"><button type="button" onClick={() => setImage(1)} aria-label="الصورة التالية" className="image-nav"><ChevronRight size={17} /></button><button type="button" onClick={() => setImage(-1)} aria-label="الصورة السابقة" className="image-nav"><ChevronLeft size={17} /></button></div></div></div>
          </FadeIn>
