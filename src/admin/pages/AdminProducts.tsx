import { Check, Edit3, ImagePlus, Plus, Search, Star, ToggleLeft, ToggleRight, Trash2, X } from 'lucide-react';
...
export default function AdminProducts() {
  ...
  const load = async () => {
    ...
    const [categoryResult, productResult] = await Promise.all([
      ...
      client.from('products').select('*, categories(id, name, slug), product_images(id, image_url, display_order)').order('display_order', { ascending: true }),
    ]);
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (urls.length) { const imageResult = await client.from('product_images').insert(urls.map((imageUrl, index) => ({ product_id: productId, image_url: imageUrl, display_order: index }))); if (imageResult.error) throw imageResult.error; }
