  ...
  return (
    <div dir="rtl" className="admin-shell min-h-screen bg-paper text-ink">
      <aside className={`admin-sidebar ${open ? 'admin-sidebar-open' : ''}`}>
        ...
        <div className="mt-12"><p className="eyebrow mb-4 text-ink/35">SAIF / التحكم</p><nav className="flex flex-col gap-1">{NAV_ITEMS.map(({ to, label, icon: Icon }) => <Link key={to} to={to} onClick={() => setOpen(false)} className={`admin-nav-link ${pathname === to ? 'admin-nav-active' : ''}`}><Icon size={17} strokeWidth={1.8} /><span>{label}</span></Link>)}</nav></div>
