...
The app has a curated local preview mode for design review. Production mode is selected automatically when `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` exist. The public client never receives a service role key. Supabase SQL/RLS/RPCs and the `create-order` Edge Function define the live security boundary.
